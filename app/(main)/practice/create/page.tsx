...
export default function PracticeCreatePage() {
  ...
  const handleSave = async (publish: boolean) => {
    if (!examData.title) {
      alert('Vui lòng nhập tiêu đề đề luyện thi')
      setStep(1)
      return
    }
    ...
    try {
      ...
      router.push('/practice/manage')
    } catch (error: any) {
      alert('Lỗi lưu đề luyện thi: ' + error.message)
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const steps = [
    { id: 1, name: 'Cấu hình Đề luyện thi' },
    { id: 2, name: 'Chọn & Soạn câu hỏi' },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 h-screen flex flex-col">
      <div className="flex items-center justify-between mb-8 flex-shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Tạo đề luyện thi</h1>
          <p className="text-sm text-gray-500 mt-1">Xây dựng nhanh đề luyện tập từ ngân hàng</p>
        </div>
            ...
            <li key={s.name} className="md:flex-1">
              <button
                ...
                className={`group flex w-full flex-col border-l-4 py-2 pl-4 transition-colors md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4 ${
                  step === s.id ? 'border-blue-600' : step > s.id ? 'border-green-600' : 'border-gray-200'
                }`}
              >
                <span className={`text-sm font-medium ${step === s.id ? 'text-blue-600' : step > s.id ? 'text-green-600' : 'text-gray-500'}`}>
                  Bước {s.id}
                </span>
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center px-4 sm:px-6 lg:px-8">
          ...
          {step < 3 ? (
            ...
            >
              ...
              {examData.is_published ? 'Lưu & Xuất bản' : 'Lưu Đề Luyện Thi'}
            </button>
          )}
        </div>
      </div>
